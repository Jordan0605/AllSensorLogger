...
public class MainActivity extends FragmentActivity{
    ...
    AsyncTask task = new LoadingDataAsyncTask();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences = MainActivity.this.getSharedPreferences(getString(R.string.PREFS_NAME), 0);
        ...
        if(idToken == null)
            idToken = VolleyController.getInstance().getUserCredentials().getIdToken();
                    ...
                    public void onSuccess(final UserProfile payload) {
                        MainActivity.this.runOnUiThread(new Runnable() {
                            public void run() {
                                ...
                                StringRequest stringRequest = new StringRequest(Request.Method.POST, "http://140.113.203.226/~andersen/GetUser.php",  new Response.Listener<String>() {
                                    @Override
                                    public void onResponse(String response) {
                                        if(response.equals("fail")){
                                            Create_User(payload.getId());
                                            Toast.makeText(MainActivity.this, "create user ", Toast.LENGTH_SHORT).show();
                                            Object[] arg = new String[]{response};
                                            task.execute(arg);
                                        }else{
...
//                                            editor.putString("Name1", r[0]);
//                                            editor.putString("CarAge1", r[1]);
//                                            editor.putString("CarType1", r[2]);
//                                            editor.putString("Device", r[3]);
//                                            editor.commit();
       ...
       /* LayoutInflater inflater = LayoutInflater.from(this);
        ...
        mTabHost.addTab(mTabHost.newTabSpec("Placeholder one")
                        .setIndicator("Main")
                , FakeFragment.class, null);
        mTabHost.addTab(mTabHost.newTabSpec("Placeholder two")
                        .setIndicator("Setup")
                , FakeFragment_2.class, null);


    }

    class LoadingDataAsyncTask extends AsyncTask<String, String, String > {
        @Override
...
//
        ...
        protected void onPostExecute(String result) {
            ...
            if(!result.equals("fail")){
                ...
                //solve problem for some user may be created but no have any carInfo, such as name = null, carType = null, carAge = 0, device = null
                if(rs_len == 1 && rs[0].equals("__0_"))
                {
                    Log.e("Result", "No carinfo for user");
                    editor.putInt("CarInfoCounter", 0);
                }
                else
                {
                    editor.putInt("CarInfoCounter", rs_len);
                    int Now_CarInfo = sharedPreferences.getInt("CarInfoNow", 0);
                    if (Now_CarInfo == 0)
                        editor.putInt("CarInfoNow", rs_len);
                    editor.commit();
                    for (int i = 0; i < rs_len; i++) {
                        String r[] = rs[i].split("_");
                        Log.v("TAG", r[0]);
                        String a = "Name" + (i + 1);
                        String b = "CarAge" + (i + 1);
                        String c = "CarType" + (i + 1);
                        String d = "Device";
                        editor.putString(a, r[0]);
                        editor.putString(c, r[1]);
                        editor.putString(b, r[2]);
                        editor.putString(d, r[3]);
                        editor.commit();
                    }
                }
...
//
    ...
    private String Create_User(final String User_id){
